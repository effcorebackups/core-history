...
и пытаются отридцать очевидные вещи.
...
хранится в директории frontend, а для backend - в директории backend).
Также всё, что кажется сложным, должно отвергаться или переделываться.
Таким образом разработчик любого уровня может легко воспринимать элементы
системы не прибегая к комментариям.
...
учёта параллельной загрузки дополнительного контента в виде JS, CSS).
