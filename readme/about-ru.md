...
Каждая строка такого файла описывает единичный атрибут сущности и может
принимать виды: "ключ", "ключ|имя_класса", "ключ: значение", "- ключ: значение".
...
через административный интерфейс). Пример PHP-файла показан ниже.
...
Хранение данных в директории "dynamic/cache" производится раздельно для каждого вида сущности
как показано на примере ниже.

Пример раздельного хранения данных:
...
Замечание: системное меню расположено в NoSQL-хранилище и никто не может
