...
namespace effcore { # cache for data--pages-storage-instance_update_by_entity_name_and_instance_id
  ...
  cache::$data['data--pages-storage-instance_update_by_entity_name_and_instance_id']->display->match = '%^(?<base>/manage/instances)/(?<action>update)/(?<entity_name>[a-z0-9_]+)/(?<instances_id>[a-z0-9_/]+)$%';
